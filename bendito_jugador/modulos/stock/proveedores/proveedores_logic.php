...
{
    return [
        'rubros' => $db->query('SELECT * FROM rubros_proveedor WHERE estado = 1 ORDER BY nombre')->fetchAll(),
        'condiciones_iva' => $db->query('SELECT * FROM condiciones_iva WHERE estado = 1 ORDER BY nombre')->fetchAll(),
        'paises' => $db->query('SELECT * FROM paises WHERE estado = 1 ORDER BY nombre')->fetchAll(),
        'provincias' => $db->query(
        ...
        'estados' => $db->query('SELECT * FROM estados_proveedor ORDER BY id_estado_proveedor')->fetchAll(),
    ];
}
...
function proveedor_validar(PDO $db, array $data, int $idProveedor = 0): array
{
    ...
    if ($data['id_estado_proveedor'] <= 0 || !proveedor_opcion_existe($db, 'estados_proveedor', 'id_estado_proveedor', (int) $data['id_estado_proveedor'], false)) {
        $errors[] = 'Debe seleccionar un estado valido.';
    }
    ...
    foreach ($optionalChecks as [$field, $table, $idColumn, $label]) {
        if ((int) $data[$field] > 0 && !proveedor_opcion_existe($db, $table, $idColumn, (int) $data[$field])) {
    ...
    try {
        $stmt = $db->prepare(
            'INSERT INTO proveedores (
                ...
                codigo_postal, telefono, email, sitio_web, id_condicion_iva, id_rubro_proveedor,
        ...
        $stmt = $db->prepare(
            'UPDATE proveedores
             SET cuit = ?, razon_social = ?, nombre_fantasia = ?, direccion = ?, id_pais = ?,
                 ...
                 sitio_web = ?, id_condicion_iva = ?, id_rubro_proveedor = ?, plazo_pago = ?,
